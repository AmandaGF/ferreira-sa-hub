...
<div class="client-header">
    ...
    <div class="client-actions">
        <?php if ($client['phone']): ?>
            <button type="button" onclick="waSenderOpen({telefone:'<?= preg_replace('/[^0-9+]/', '', $client['phone']) ?>',nome:<?= e(json_encode($client['name'])) ?>,clientId:<?= (int)$client['id'] ?>,mensagem:''})" class="btn btn-success btn-sm" style="border:none;cursor:pointer;">💬 WhatsApp</button>
        <?php endif; ?>
        <?php if (!$isReadOnly): ?>
...
<div class="card" style="margin-bottom:1.25rem;border-left:5px solid <?= $borda ?>;background:<?= $bg ?>;">
    <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;background:transparent;border-bottom:1px solid rgba(0,0,0,.06);">
        <div>
            <h3 style="margin:0;"><?= $temVencido ? '⚠️ Situação Financeira do Cliente — INADIMPLENTE' : '💰 Situação Financeira do Cliente' ?></h3>
