...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ═══ Import automático de andamentos do email PJe ═══
    ...
    // CNJ) e importamos todos os movimentos pra case_andamentos (dedup por MD5).
    //
    // OBS: as PARTES são pré-preenchidas no FORMULÁRIO via $partesPreFillEmail (GET-time),
    // então quando o POST chega, os polos já vêm como linhas em $_POST['partes_nome'] e
    // o INSERT existente das partes (loop nos partes_nome[]) já cuida deles. Não fazemos
    // INSERT separado aqui pra evitar duplicatas quando Amanda edita os nomes pré-fillados.
    $importadosPje = 0;
    if ($case_number !== '') {
        try {
            ...
            if (!empty($pendRows)) {
                ...
                // Import dos movimentos via IMAP
