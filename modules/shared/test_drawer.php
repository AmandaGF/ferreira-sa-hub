<?php
$_apiUrl = url('modules/shared/card_api.php');
$_actUrl = url('modules/shared/card_actions.php');
$_opUrl = url('modules/operacional/api.php');
$_tk = CSRF_TOKEN_NAME . '=' . csrf_token();
?>
...
<script>
console.log('[CD] init');
var D=null,T='geral',AU='<?php echo $_apiUrl;?>',XU='<?php echo $_actUrl;?>',OU='<?php echo $_opUrl;?>',TK='<?php echo $_tk;?>';
function cdOpen(p){
console.log('[CD] open',p);
var o=document.getElementById('cdOv'),n=document.getElementById('cdPn');
o.style.display='block';n.style.display='flex';
setTimeout(function(){n.style.right='0'},10);
document.getElementById('cdBd').innerHTML='<div style="text-align:center;padding:3rem;color:#94a3b8">Carregando...</div>';
var x=new XMLHttpRequest();
x.open('GET',AU+'?'+p);
x.onload=function(){
try{D=JSON.parse(x.responseText);if(D.error){document.getElementById('cdBd').textContent=D.error;return}cdBuild()}
catch(e){document.getElementById('cdBd').textContent='Erro: '+e.message;console.error(e)}
};x.onerror=function(){document.getElementById('cdBd').textContent='Erro de rede'};x.send()
}
function cdClose(){
var n=document.getElementById('cdPn');n.style.right='-520px';
setTimeout(function(){document.getElementById('cdOv').style.display='none';n.style.display='none'},300)
}
...
var hd='<div style="display:flex;justify-content:space-between"><div>';
hd+='<div style="font-size:1.05rem;font-weight:800">'+E(c.name)+'</div>';
var m=[];if(c.cpf)m.push('CPF: '+c.cpf);if(c.phone)m.push(c.phone);
hd+='<div style="font-size:.73rem;color:rgba(255,255,255,.6);margin-top:.15rem">'+m.join(' \u00B7 ')+'</div>';
hd+='</div><button onclick="cdClose()" style="background:none;border:none;color:#fff;font-size:1.2rem;cursor:pointer">X</button></div>';
var bg='';
if(l)bg+='<span class="cb" style="background:#6366f1">'+(sl[l.stage]||l.stage)+'</span> ';
...
th+='<button class="ct'+(t===T?' on':'')+'" onclick="T=\''+t+'\';document.querySelectorAll(\'.ct\').forEach(function(b){b.classList.remove(\'on\')});this.classList.add(\'on\');cdTab()">'+tl[t]+'</button>';
});
...
if(D.form_data){h+='<div class="cs"><h5>Formulario</h5>';var sk=['nome','name','client_name','client_phone','client_email','email','celular','phone','cpf','form_type','protocol_original','protocol','protocolo','id','created_at','updated_at','ip','ip_address','user_agent','data_envio','payload_json','totais'];for(var k in D.form_data){if(sk.indexOf(k)>=0)continue;var fv=D.form_data[k];if(fv===null||fv===''||typeof fv==='object')continue;h+=R(k.replace(/_/g,' '),fv)}h+='</div>'}
h+='<div class="cs"><h5>Comentarios</h5><textarea id="cdCm" style="width:100%;font-size:.8rem;padding:6px;border:1.5px solid #e5e7eb;border-radius:6px;min-height:45px;resize:vertical" placeholder="Escrever..."></textarea><button onclick="cdCom()" style="background:#B87333;color:#fff;border:none;padding:3px 12px;border-radius:5px;font-size:.7rem;font-weight:600;cursor:pointer;margin-top:3px">Comentar</button>';
(D.comments||[]).forEach(function(cm){h+='<div style="padding:5px 0;border-top:1px solid #f3f4f6;margin-top:3px"><strong style="font-size:.73rem">'+E(cm.user_name)+'</strong> <span style="font-size:.6rem;color:#94a3b8">'+FD(cm.created_at)+'</span><div style="font-size:.78rem;margin-top:1px">'+E(cm.message)+'</div></div>'});
h+='</div>'
}else if(T==='comercial'&&l){
h+='<div class="cs"><h5>Contrato</h5>'+R('Valor',l.valor_acao)+R('Forma Pagamento',l.forma_pagamento)+R('Vencimento',l.vencimento_parcela)+R('Nome Pasta',l.nome_pasta)+R('Pendencias',l.pendencias)+R('Convertido',l.converted_at?FD(l.converted_at):null)+'</div>';
h+='<div class="cs"><h5>Historico Pipeline</h5>';
(D.pipeline_history||[]).forEach(function(ph){h+='<div style="padding:3px 0;border-bottom:1px solid #f3f4f6;font-size:.75rem"><strong>'+E(ph.user_name?ph.user_name.split(' ')[0]:'')+'</strong> moveu para <strong>'+(sl[ph.to_stage]||ph.to_stage)+'</strong> <span style="color:#94a3b8;font-size:.62rem">'+FD(ph.created_at)+'</span></div>'});
if(!(D.pipeline_history||[]).length)h+='<div style="color:#94a3b8">Nenhuma</div>';
h+='</div>'
...
h+='<div class="cs"><h5>Tarefas ('+(D.tasks||[]).length+')</h5>';
(D.tasks||[]).forEach(function(t){var dn=t.status==='feito';h+='<div style="display:flex;align-items:center;gap:5px;padding:2px 0;font-size:.77rem"><span style="color:'+(dn?'#059669':'#d1d5db')+'">'+(dn?'[x]':'[ ]')+'</span><span style="'+(dn?'text-decoration:line-through;color:#94a3b8':'')+'">'+E(t.title)+'</span></div>'});
h+='</div><div class="cs"><h5>Andamentos</h5>';
(D.andamentos||[]).slice(0,5).forEach(function(a){h+='<div style="padding:3px 0;border-bottom:1px solid #f3f4f6;font-size:.75rem"><strong>'+FD(a.data_andamento)+'</strong> '+E(a.tipo)+' <span style="color:#6b7280">'+E(a.descricao?a.descricao.substring(0,100):'')+'</span></div>'});
h+='</div>'
}else if(T==='docs'){
var pn=(D.docs_pendentes||[]).filter(function(x){return x.status==='pendente'}),rv=(D.docs_pendentes||[]).filter(function(x){return x.status!=='pendente'});
if(pn.length){h+='<div class="cs"><h5 style="color:#dc2626">Pendentes ('+pn.length+')</h5>';pn.forEach(function(dp){h+='<div style="display:flex;align-items:center;gap:6px;padding:5px;margin-bottom:3px;background:#fef2f2;border-radius:5px;border-left:3px solid #dc2626"><div style="flex:1;font-weight:600;color:#dc2626;font-size:.8rem">'+E(dp.descricao)+'</div><button onclick="cdDoc('+dp.id+')" id="db'+dp.id+'" style="background:#059669;color:#fff;border:none;padding:3px 8px;border-radius:4px;font-size:.68rem;font-weight:600;cursor:pointer">Recebido</button></div>'});h+='</div>'}
...
h+='<div class="cs"><h5>Pecas ('+(D.pecas||[]).length+')</h5>';
(D.pecas||[]).forEach(function(p){h+='<div style="padding:2px 0;border-bottom:1px solid #f3f4f6;font-size:.77rem">'+E(p.titulo||'Peca')+'</div>'});
if(!(D.pecas||[]).length)h+='<div style="color:#94a3b8;font-size:.77rem">Nenhuma</div>';h+='</div>'
}else if(T==='agenda'){
h+='<div class="cs"><h5>Compromissos</h5>';
(D.compromissos||[]).forEach(function(ev){h+='<div style="padding:3px 0;border-bottom:1px solid #f3f4f6"><div style="font-weight:600;font-size:.77rem">'+E(ev.titulo)+'</div><div style="font-size:.67rem;color:#6b7280">'+FD(ev.data_inicio)+' - '+E(ev.tipo)+'</div></div>'});
if(!(D.compromissos||[]).length)h+='<div style="color:#94a3b8;font-size:.77rem">Nenhum</div>';h+='</div>'
}else if(T==='historico'){
(D.historico||[]).forEach(function(hi){h+='<div style="padding:3px 0;border-bottom:1px solid #f3f4f6;font-size:.75rem"><strong>'+FD(hi.date)+'</strong> '+hi.icon+' '+E(hi.text)+'</div>'});
if(!(D.historico||[]).length)h+='<div style="color:#94a3b8;text-align:center;padding:1rem">Nenhum</div>'
}else{h='<div style="color:#94a3b8;padding:2rem;text-align:center">Sem dados</div>'}
document.getElementById('cdBd').innerHTML=h
}
function cdCom(){var t=document.getElementById('cdCm');if(!t||!t.value.trim())return;var x=new XMLHttpRequest();x.open('POST',XU);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');x.onload=function(){try{var r=JSON.parse(x.responseText);if(r.ok&&r.comment){if(!D.comments)D.comments=[];D.comments.unshift(r.comment);t.value='';cdTab()}}catch(e){}};x.send('action=add_comment&client_id='+D.client_id+'&case_id='+(D.case_id||0)+'&lead_id='+(D.lead_id||0)+'&message='+encodeURIComponent(t.value))}
function cdDoc(id){if(!confirm('Confirmar recebimento?'))return;var b=document.getElementById('db'+id);if(b){b.textContent='...';b.disabled=true}var x=new XMLHttpRequest();x.open('POST',OU);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');x.onload=function(){cdOpen('case_id='+D.case_id);setTimeout(function(){T='docs';cdTab()},500)};x.send('action=resolve_doc&doc_id='+id+'&case_id='+D.case_id+'&'+TK)}
