 ...
 * Se receber protocolo existente, faz UPDATE (permite cliente revisar)
 */
...
try {
    ...
    // Verificar se é uma ATUALIZAÇÃO (protocolo existente)
    $protocoloExistente = trim($dados['_protocolo'] ?? '');
    $isUpdate = false;

    if ($protocoloExistente) {
        $stmt = $pdo->prepare("SELECT id FROM form_submissions WHERE protocol = ? AND form_type = 'despesas_mensais'");
        $stmt->execute(array($protocoloExistente));
        $existente = $stmt->fetch();

        if ($existente) {
            // UPDATE — sobrescrever dados
            $pdo->prepare(
                "UPDATE form_submissions SET client_name = ?, client_phone = ?, payload_json = ?, ip_address = ?, updated_at = NOW() WHERE protocol = ? AND form_type = 'despesas_mensais'"
            )->execute(array($nome, $whatsapp, $payload_json, $ip, $protocoloExistente));

            responder(true, 'Dados atualizados com sucesso!', array(
