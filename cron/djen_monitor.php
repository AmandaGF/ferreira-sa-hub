 ...
 *
 * PROPÓSITO:
 *   Roda 2× por dia (08h e 19h) via cron do cPanel. Para cada
 *   OAB do escritório, puxa as publicações do dia-alvo na API
 *   pública do DJEN (comunicaapi.pje.jus.br), pede ao Claude
 *   um resumo + orientação curta, monta o texto no formato do
 *   parser do Hub e posta no endpoint /api/djen_ingest.php.
 *
 *   Tudo que já tem pasta é importado automaticamente (com
 *   tarefa de prazo, evento na agenda e andamento trancado).
 *   O que não tem pasta fica em djen_pending para Amanda
 *   revisar no dashboard.
 *
 * EXECUÇÃO:
 *   1. Pelo cron (cPanel):
 *      /usr/bin/php /caminho/.../cron/djen_monitor.php --horario=auto
 *   2. Manual pelo dashboard (shell_exec via HTTP autenticado):
 *      php djen_monitor.php --horario=manual --data=2026-04-22 --token=<CLAUDIN_MANUAL_TOKEN>
 *   3. Manual pelo terminal (se tiver SSH):
 *      php djen_monitor.php --horario=08
 *
 * ARGUMENTOS:
 *   --horario=08|19|manual|auto  (obrigatório)
 *     08     = puxa o dia útil anterior (execução de manhã)
 *     19     = puxa o dia de hoje (execução de noite)
 *     manual = exige --data=YYYY-MM-DD
 *     auto   = decide 08 ou 19 conforme a hora atual
 *   --data=YYYY-MM-DD  (obrigatório quando --horario=manual)
 *   --token=<segredo>  (obrigatório quando não-CLI)
 *
 * O QUE A AMANDA PRECISA EDITAR AQUI:
 *   NADA. Todo parâmetro mora em claudin_config.php.
 *
 * DEPENDÊNCIAS:
 *   - claudin_config.php (Passo 2)
 *   - core/database.php (PDO)
 *   - Endpoint /api/djen_ingest.php (já em produção)
 *   - Tabela claudin_runs (criada no Passo 1)
 *
 * ============================================================
 */

// Marca que o config pode ser carregado (bloqueia execução via navegador sem contexto válido)
...
// ============================================================
// 1. Autenticação de execução
// ============================================================
// Script só pode rodar como:
//  - CLI (cron do servidor, SSH, shell_exec)
//  - Ou via HTTP com token válido (dashboard chamando localmente)
...
// ============================================================
// 2. Parse dos argumentos (aceita CLI e HTTP)
// ============================================================
function claudin_arg($chave, $default = null) {
    // HTTP: aceita via $_REQUEST
    if (isset($_REQUEST[$chave])) return $_REQUEST[$chave];
    // CLI: aceita --chave=valor em argv
    global $argv;
...
$horario = claudin_arg('horario', 'auto');
...
// "auto" decide com base na hora local
if ($horario === 'auto') {
    $hora = (int)date('H');
    $horario = ($hora < 12) ? '08' : '19';
}

// Determina a data-alvo a ser consultada
if ($horario === 'manual') {
    if (!$dataAlvoArg || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $dataAlvoArg)) {
        echo "ERRO: --horario=manual exige --data=YYYY-MM-DD\n";
        exit(1);
    }
    $dataAlvo = $dataAlvoArg;
} elseif ($horario === '08') {
    // Puxa o último dia útil anterior a hoje
    $dataAlvo = claudin_dia_util_anterior(date('Y-m-d'));
} else {
    // horario=19 → puxa hoje
    $dataAlvo = date('Y-m-d');
}

// ============================================================
// 3. Helpers — datas, log, e-mail, HTTP
// ============================================================

/**
 * Retorna o dia útil imediatamente anterior a $dataRef (Y-m-d).
 * Considera apenas weekend (sábado/domingo). Feriados não são
 * tratados aqui — se alguma execução cair em feriado, o robô
 * vai puxar o dia anterior mesmo assim (e virá vazio, sem dano).
 */
...
/**
 * Calcula data fatal somando N dias úteis a partir de $dataInicio.
 * Regra CPC art. 224 §3º: termo inicial é o primeiro dia útil
 * seguinte à data de disponibilização. Depois conta N dias úteis.
 * Feriados não considerados (orientação é aproximada — advogado
 * revisa no dashboard).
 */
function claudin_calcular_data_fatal($dataDisp, $diasUteis) {
    if ($diasUteis <= 0) return null;
    $d = new DateTime($dataDisp);
    // Avança até o próximo dia útil (data de ciência)
    do {
        $d->modify('+1 day');
    } while ((int)$d->format('N') >= 6);
    // Avança mais N dias úteis
    $cont = 0;
 ...
 * Renomeia claudin.log → claudin.log.1 e recomeça.
 */
...
/**
 * Grava linha no log. Formato: [YYYY-MM-DD HH:MM:SS] [PID] [horario] mensagem
 */
...
/**
 * Envia e-mail de alerta. Usa mail() nativo. Se falhar, loga
 * mas não aborta (falha de SMTP não deve parar o robô).
 */
...
/**
 * Faz POST HTTP com cURL e retorna [http_code, body].
 * Timeout e headers passados por parâmetro.
 */
...
// ============================================================
// 4. API DJEN — paginação por OAB
// ============================================================

/**
 * Consulta todas as publicações de uma OAB/UF para uma data
 * específica. Pagina de 100 em 100 até esvaziar.
 * Retorna array plano de items (cada um é associativo).
 */
function claudin_buscar_djen($numeroOab, $uf, $dataAlvo) {
    $items = array();
    $pagina = 1;
    $maxPaginas = 50; // trava de segurança: 50 × 100 = 5000 pubs/OAB/dia
    ...
    while ($pagina <= $maxPaginas) {
        ...
        // A API pode retornar os items em chaves diferentes; cobre as variações
        $lote = array();
        ...
        // Se veio menos que o tamanho da página, é a última
...
// ============================================================
// 5. Claude Haiku — resumo + orientação com retry
// ============================================================

/**
 * Chama a API Anthropic pedindo resumo e orientação.
 * Implementa retry com backoff exponencial (30s, 60s, 120s).
 * Se após 3 tentativas ainda falhar, retorna array com marca
 * de falha — nunca lança exception (não pode abortar o lote).
 */
function claudin_chamar_claude($item, $dataFatalSugerida) {
    ...
    for ($tentativa = 1; $tentativa <= ANTHROPIC_RETRY_TENTATIVAS; $tentativa++) {
        ...
        if ($resp['code'] >= 200 && $resp['code'] < 300) {
            ...
            if ($texto) {
                // Extrai JSON (às vezes vem envolto em markdown)
    ...
    // Todas as tentativas falharam — marca manual mas NÃO aborta
    return array(
...
// ============================================================
// 6. Detecção de segredo de justiça
// ============================================================
...
// ============================================================
// 7. Monta o bloco de texto no formato do parser
// ============================================================
...
// ============================================================
// 8. Envia payload consolidado ao endpoint do Hub
// ============================================================
...
// ============================================================
// 10. Exception handler global — nunca deixa escapar
// ============================================================
set_exception_handler(function($e) use (&$pdo, $horario, $dataAlvo) {
    claudin_log('EXCEPTION FATAL: ' . $e->getMessage());
    try {
        if (isset($pdo) && $pdo) {
            claudin_salvar_run(
                $pdo, date('Y-m-d H:i:s'), $dataAlvo, $horario,
    ...
    claudin_enviar_email(
        'Claudin ' . $horario . 'h: ERRO de infraestrutura',
...
// 11.1 — Coleta todos os items, deduplicando por hash
$itemsPorHash = array();
foreach (OABS_MONITORADAS as $oab) {
...
// 11.2 — Se vazio, grava run e manda alerta
if ($totalUnico === 0) {
    ...
    claudin_log("Nenhuma publicação encontrada para {$dataAlvo}. Notificando.");
    claudin_enviar_email(
...
// 11.3 — Pra cada pub, pede resumo/orientação ao Claude e monta bloco
$blocos = array();
$prazoDefault = 15; // dias úteis (maioria dos atos relevantes)
...
// 11.4 — Envia ao endpoint do Hub e interpreta resposta
try {
...
// 11.5 — Verifica invariante: total_parsed == imported + duplicated + pending + errors
...
$status = 'ok';
if (!$invarianteOk || $contadores['errors'] > 0) {
    $status = 'falha';
} elseif ($contadores['pending'] > 0) {
    $status = 'parcial';
}
...
if ($precisaEmail) {
    $assunto = 'Claudin ' . $horario . 'h: ' .
        ($contadores['pending'] > 0 ? $contadores['pending'] . ' pendentes precisam de vinculação'
            : ($contadores['errors'] > 0 ? 'ERROS na importação'
                : 'atenção necessária'));
    ...
    // Lista CNJs pendentes com resumo de 1 linha
